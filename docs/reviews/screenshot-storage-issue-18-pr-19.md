...
The app previously vendored plugin 0.13.1 while the current public plugin release is 0.13.2. The companion starts from the verified 0.13.2 release and prepares an **unreleased 0.13.3 / MCP 0.3.3 candidate**, preserving newer tool metadata and skill safety guidance. Vendor synchronization now copies the skill with the server and records hashes for both. Xcode verifies both before packaging. Worktree builds resolve their own MCP server instead of an enclosing checkout's older server.

Updating the app does not update installed Claude Code, Codex, or OMP marketplace caches. Existing plugins already support custom primary screenshot paths; attachment delivery requires the companion update. No public release, installed-plugin update, merge, or push is part of this local implementation.

## Validation

- **106 focused Swift tests passed:** 96 covering storage, annotation round trips, image cleanup, leases, settings, and exports; 10 covering worktree server selection and plugin-version consistency.
- **228 plugin tests passed:** MCP 90, hooks 101, wake 37. Runtime bundles built; wake typecheck, schema fixtures, public version consistency, notices, skill validation, and Claude marketplace validation passed.
...
- **Package provenance:** server and skill inside the final app match the SHA-256 values from clean companion commit `10212dc0bb509167217dff1ce782d6c0a0231bc4` on `codex/screenshot-storage`. The companion worktree is `/Users/mete/Developer/remarc-agent-plugins-worktrees/screenshot-storage`.

Remaining limits: live Screen Recording capture through the OS was not exercised; real PNG writes, annotation edits, exported paths, and MCP image delivery were tested. MCP's additional standalone `tsc --noEmit` check reports the pre-existing `plugins/shared/notify.ts:19` TS2769 error, reproduced on the unchanged 0.13.2 baseline. No live marketplace client was upgraded to the candidate, and this is not a public release verification.
